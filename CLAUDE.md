...
1. **Suppression d'organisation : stats + confirmation sécurisée** — cadré le 2026-08-21 : la modale de suppression (`SuperAdminPage.tsx`) n'affiche qu'un texte générique aujourd'hui. Ajout du comptage `adherents` (absent du fetch actuel, à côté de `nb_participants`/donateurs et `nb_dons` déjà calculés) + affichage des 3 chiffres dans la modale + confirmation façon GitHub (saisie du nom exact de l'organisation pour activer le bouton Supprimer). Faible complexité, sensible (garde-fou contre suppression accidentelle de toutes les données d'une association). [Trello](https://trello.com/c/emhCr0D6)
2. **Nettoyer la dette lint** (36 erreurs/avertissements, revérifié 2026-08-20 — était 32 le 2026-08-11) — cadré : nettoyage complet en une PR, majorité (29/36) mécanique (`react-hooks/set-state-in-effect`). [Trello](https://trello.com/c/dep6US2K)
3. **Priorité 4 — Envoi par email des reçus fiscaux** — cadré le 2026-08-20 : bouton manuel individuel dans `RecusFiscauxPage.tsx`, PDF en pièce jointe via Resend, pose `recus_fiscaux.email_envoye_at`. Prérequis Resend levé depuis le 2026-08-15. [Trello](https://trello.com/c/Do9GVjOt)
...
12. **Sélection d'un adhérent à la saisie d'un don + doublonnement** — prio basse (dit explicitement par l'utilisateur), hors scope V1 du module Adhérents. Revérifié 2026-08-20 (toujours pas de lien en base adherents↔profils_participant) : cadrage volontairement resté léger vu la priorité, un cadrage détaillé complet restera nécessaire avant tout dev (mécanisme de dédoublonnage non trivial). [Trello](https://trello.com/c/RtRY8ltX)
13. **OCR scan de carte adhérent** — pas encore cadré, discussion préliminaire seulement, intégration API OCR externe, besoin niche déjà partiellement couvert par la recherche nom/prénom (livrée). [Trello](https://trello.com/c/zVBOjAWk)
14. **Priorité 5 — Export comptable enrichi** (recadré 2026-08-20, ex-"Export FEC") — un vrai FEC (partie double, plan de comptes) serait un chantier disproportionné vu l'absence totale de modélisation comptable dans Mothana ; objectif ramené à enrichir le CSV dons existant. Roadmap lointaine, besoin concret toujours non confirmé. [Trello](https://trello.com/c/W3GCYUOt)
15. **Priorité 5 — Brique événements/coupons + portefeuille virtuel** (Pagode Coupon) — clarifié 2026-08-20 : précommande/prépaiement en ligne pour les événements de la pagode (dimensionner les achats, avance de trésorerie), deux mécanismes retenus (coupon = droit à un article précis, ET portefeuille virtuel = solde rechargeable). Un projet séparé existe déjà (Supabase "pagode-coupon", en pause), pas encore inspecté — prérequis avant cadrage technique. [Trello](https://trello.com/c/C9A5B9jr)
16. **Priorité 5 — Gestion abonnements/plans** (facturation Samakan) — clarifié 2026-08-20 : facturation SaaS de Samakan lui-même (les organisations clientes paient un abonnement), pas une fonctionnalité pour les donateurs des associations. Aucun modèle économique décidé à ce jour, roadmap lointaine. [Trello](https://trello.com/c/cfKF8BNw)
