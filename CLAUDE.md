...
Décisions actées :
- Second projet Supabase permanent ("staging"), palier gratuit pour démarrer (bascule vers Pro à réévaluer si le palier gratuit se montre limitant, ex. mise en pause après inactivité).
- **Schéma seul + données synthétiques** — aucune donnée réelle (PII donateurs/adhérents) n'est jamais copiée vers staging, pas de script d'anonymisation nécessaire.
- Synchro quotidienne = reset du schéma staging depuis le schéma prod (source de vérité, DDL uniquement), puis réamorçage des données synthétiques via un script idempotent.
- Horodatage de chaque synchro dans un fichier suivi en repo ; nouvelle routine de fin de session à ajouter (vérifier qu'une synchro a eu lieu le jour même, sinon en déclencher une).
- Nouveau workflow de dev une fois en place : migrations + Edge Functions testées sur staging en premier, promotion vers prod seulement après merge de la PR.
