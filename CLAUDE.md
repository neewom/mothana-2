...
Ordre reclassé le 2026-08-17 (jugement agent, mix importance/complexité — pas un simple reflet de l'ordre d'ajout) :

1. **Mire de connexion personnalisée par organisation** (admin + bénévole) — cadré le 2026-08-10, même mécanique que le formulaire d'adhésion (header/footer/css éditables, URL `/connexion/{slug}` et `/login/benevole/{slug}` réutilisant le slug existant). Valeur UX/confiance multi-org. [Trello](https://trello.com/c/gkOuH3uh)
2. **Configurer le contenu des mails de création de compte / reset password** — pas cadré, mais polish probablement contenu sur un parcours critique déjà en prod (templates Resend). [Trello](https://trello.com/c/Qf8mdFTz)
...
4. **Priorité 4 — Envoi email des reçus fiscaux** — pas cadré. Prérequis Resend désormais levé (compte en place depuis le 2026-08-15), complète le pipeline Cerfa. [Trello](https://trello.com/c/Do9GVjOt)
5. **Double opt-in email** avant ratification d'une demande d'adhésion — cadré le 2026-08-08. Prérequis Resend désormais levé. Complexité un peu plus élevée que les items précédents (nouveau token/route publique). [Trello](https://trello.com/c/0gt0LIVU)
6. **Bandeau visuel distinguant l'environnement recette de la prod** — pas cadrée, complexité faible, utilité accrue depuis que la recette sert à des clients réels. [Trello](https://trello.com/c/hSogRI1Y)
7. **Modèles réutilisables pour les campagnes mailing** — évoqué le 2026-08-14, pas cadré, amélioration de confort sur une feature déjà livrée. [Trello](https://trello.com/c/BfHOUb3v)
8. **Nettoyer la dette lint** (32 erreurs/avertissements pré-existants, +5 depuis, cf. `react-hooks/set-state-in-effect`) — pas cadré, dette technique sans valeur utilisateur directe, pas urgent. [Trello](https://trello.com/c/dep6US2K)
9. **Contrainte d'unicité `id_externe` par organisation sur les activités** — point non bloquant, pas cadré, aucun incident constaté à date. [Trello](https://trello.com/c/WkPAb7K7)
10. **Sélection d'un adhérent à la saisie d'un don + doublonnement** — prio basse (dit explicitement par l'utilisateur), hors scope V1 du module Adhérents, logique de dédoublonnage non triviale. [Trello](https://trello.com/c/RtRY8ltX)
...
12. **Priorité 5 — Export FEC / intégrations comptables** — roadmap lointaine, pas cadrée, forte complexité (formalisme légal), besoin réel non confirmé à ce stade. [Trello](https://trello.com/c/W3GCYUOt)
13. **Priorité 5 — Brique événements/coupons** (Pagode Coupon) — roadmap lointaine, pas cadrée, nouveau domaine fonctionnel entier. [Trello](https://trello.com/c/C9A5B9jr)
14. **Priorité 5 — Gestion abonnements/plans** — roadmap lointaine, pas cadrée, scope encore flou même au niveau concept. [Trello](https://trello.com/c/cfKF8BNw)
