...
1. **Nettoyer la dette lint** (36 erreurs/avertissements, revérifié 2026-08-20 — était 32 le 2026-08-11) — cadré : nettoyage complet en une PR, majorité (29/36) mécanique (`react-hooks/set-state-in-effect`). [Trello](https://trello.com/c/dep6US2K)
2. **Priorité 4 — Envoi par email des reçus fiscaux** — cadré le 2026-08-20 : bouton manuel individuel dans `RecusFiscauxPage.tsx`, PDF en pièce jointe via Resend, pose `recus_fiscaux.email_envoye_at`. Prérequis Resend levé depuis le 2026-08-15. [Trello](https://trello.com/c/Do9GVjOt)
3. **Support multi-organisation pour un compte admin** (rattachement + sélecteur) — cadré le 2026-08-22, détecté en testant la PR #73 (branding emails) : `create-admin` échoue avec l'erreur brute de Supabase Auth si l'email correspond à un compte déjà admin d'une autre organisation. Le schéma (`profils_organisation`, `unique(utilisateur_id, organisation_id)`) prévoit ce cas mais rien dans l'appli ne l'exploite — `AuthContext.fetchOrganisationId` fait un `.single()` qui casserait la connexion si un compte avait plusieurs lignes. Déjà listé "hors périmètre MVP" dans `docs/cadrage-mothana.md`. Décisions : sélecteur d'organisation permanent dans le header (`AdminLayout.tsx`), confirmation explicite obligatoire avant de rattacher un compte existant à une nouvelle organisation (évite qu'une faute de frappe d'email donne accès aux données d'une organisation à la mauvaise personne), email "nouvel accès" informatif (pas de lien d'invitation, la personne a déjà un mot de passe). Besoin surtout pratique pour l'utilisateur (recette/démos) à ce stade, pas de vrai besoin client confirmé — version simple. **Dev reporté à plus tard (décision explicite utilisateur le 2026-08-22)**, contourné en testant avec un email différent. [Trello](https://trello.com/c/WtLrSLGW)
4. **Vraie documentation utilisateur (guides / FAQ)** — détectée le 2026-08-21 en scindant la page de tour produit `/decouvrir` (terminée, voir "✅ Terminé"), pas encore cadrée. Piste évoquée par l'utilisateur : format FAQ plutôt qu'un mode d'emploi linéaire, avec recherche par mot-clé. [Trello](https://trello.com/c/5pwUxgqA)
5. **Créer et maintenir des `agents.md` calqués sur les `CLAUDE.md`** (pour pouvoir switcher entre Codex et Claude Code) — carte détectée le 2026-08-21, pas encore cadrée. [Trello](https://trello.com/c/0XmhK0iv)
6. **Mire de connexion personnalisée par organisation** (admin + bénévole) — cadré le 2026-08-10, revérifié 2026-08-20 (toujours aligné). Même mécanique que le formulaire d'adhésion (header/footer/css éditables, URL `/connexion/{slug}` et `/login/benevole/{slug}` réutilisant le slug existant). Valeur UX/confiance multi-org. [Trello](https://trello.com/c/gkOuH3uh)
7. **Pièces jointes sur un don** (justificatifs) — cadré le 2026-08-09, revérifié 2026-08-20 (toujours aligné). Table `dons_fichiers`, bucket Storage privé + URL signée, images/PDF 10 Mo max, upload côté admin (`DonModal`) et bénévole (`BenevolePage`). [Trello](https://trello.com/c/G1MFP7Ao)
