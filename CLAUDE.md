...
**Backlog non priorisé (ajouté 2026-07-25)**
- **Réaffectation de don à un autre participant** (correction d'erreur de saisie) — risque identifié et confirmé par vérification du schéma/code avant d'ajouter cet item : `recus_fiscaux.montant_total` n'est **pas** calculé à la volée, c'est un instantané figé au moment de la génération (`SUM(dons.montant)` calculé une seule fois dans `generate-recu/index.ts`, pas de FK entre `dons` et `recus_fiscaux` — le lien est seulement `profil_participant_id` + `annee`). Conséquence : réaffecter un don à un autre participant après qu'un reçu fiscal a déjà été généré pour l'année concernée (source **ou** destination) désynchronise silencieusement le montant du reçu, sans que rien ne le signale. Point de blocage à prévoir : interdire ou avertir fortement si un `recus_fiscaux` existe déjà pour (participant source, année du don) — et si le participant destination a lui aussi déjà un reçu pour cette année. Vigilance particulière si `email_envoye_at` est renseigné (reçu déjà envoyé au donateur — document fiscal déjà émis, une correction silencieuse ne suffit pas). À cadrer plus précisément avant implémentation (interdiction totale vs. avertissement + régénération manuelle du reçu)
- **Champ de recherche pour les activités** — remplacer le `<select>` activité par un input de recherche sur l'ensemble des pages concernées, même pattern que `ParticipantAutocomplete` (déjà utilisé pour les noms dans `DonModal`/`DonsPage`)
- **Affichage cassé des dons d'un participant** (page Participants, détail participant) — avec beaucoup de dons, la liste et les CTA du bas sortent de la fenêtre. Nouveau comportement prévu : le clic ouvre un détail avec un nombre limité d'informations + CTA "Voir plus de détails" qui ouvre une modale séparée avec le détail exhaustif
- **Lenteur de chargement sur Wat Velouvanaram** (organisation avec beaucoup de données — 3336 participants importés) — à investiguer, cause non identifiée pour l'instant (pagination déjà en place via `fetchAllRows`, mais possible goulot ailleurs)
- **Page Activités : filtre + pagination** — ajouter un input de filtrage (même pattern que le point recherche activité ci-dessus) et une pagination, aligner le style de l'en-tête sur celui du tableau `ParticipantsPage` (input + les 2 CTA à droite, dans l'en-tête du tableau plutôt qu'en dehors)
