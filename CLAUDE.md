...
Ordre reclassé le 2026-08-20 (jugement agent, mix importance/complexité — pas un simple reflet de l'ordre d'ajout) :
...
4. **Mire de connexion personnalisée par organisation** (admin + bénévole) — cadré le 2026-08-10, revérifié 2026-08-20 (toujours aligné). Même mécanique que le formulaire d'adhésion (header/footer/css éditables, URL `/connexion/{slug}` et `/login/benevole/{slug}` réutilisant le slug existant). Valeur UX/confiance multi-org. [Trello](https://trello.com/c/gkOuH3uh)
5. **Afficher le numéro d'adhérent dans la vérification bénévole (nom/prénom)** — cadré le 2026-08-20 : ajout de `id_externe` au retour de la RPC `search_adherents_verification` + affichage "N° adhérent : …" dans `BenevoleVerificationAdherent.tsx`, masqué si vide. Complexité faible. [Trello](https://trello.com/c/WQ4uHU2w)
6. **Identifier l'espace bénévole + l'organisation dans le header** (`/benevole`) — cadré le 2026-08-20 : header sur 2 lignes, ligne 1 inchangée ("Samakan" + Quitter), ligne 2 "Espace bénévole — *Organisation*" (même pattern de fetch que `AdminLayout.tsx`). Complexité faible. [Trello](https://trello.com/c/ZZFYupcd)
7. **Bandeau visuel distinguant l'environnement recette de la prod** — cadré le 2026-08-20 : détection par `window.location.hostname` (pas de nouvelle variable d'env), affiché dans AdminLayout ET BenevolePage, style ambre. Complexité faible. [Trello](https://trello.com/c/hSogRI1Y)
