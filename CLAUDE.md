...
- **Désinscription mailing (RGPD)** (terminé 2026-08-18, PR #65 dev) : opt-out par lien (`adherents.mailing_opt_out`/`mailing_opt_out_at`/`mailing_unsubscribe_token`), route publique `/desinscription?token=...` + Edge Function `unsubscribe-mailing` (réponse générique anti-énumération), footer de désinscription injecté automatiquement par `send-mailing-brevo` (lien personnalisé par destinataire via les params Brevo), toggle "Ne pas contacter par email" dans `AdherentModal`. Bug corrigé au passage : la RPC `search_adherents` (liste adhérents) ne renvoyait pas les nouveaux champs, laissant la checkbox toujours décochée à l'ouverture. Validé de bout en bout sur staging (envoi réel via Brevo, exclusion confirmée par les logs Brevo, désinscription testée par l'utilisateur lui-même)
