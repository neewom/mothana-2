...
1. **Corriger le calcul de hauteur mobile (100vh → dvh)** — cadré le 2026-08-22 : cause racine confirmée, `min-h-screen`/`h-screen` Tailwind (= `100vh`) ne tient pas compte des barres dynamiques des navigateurs mobiles (barre d'adresse Safari iOS, barre d'onglets Android Chrome), d'où scroll inutile voire contenu masqué (`AdminLayout.tsx` le plus à risque, `h-screen` fixe + `overflow-hidden`). Fix : basculer sur les utilitaires `dvh` de Tailwind (dispo nativement depuis 3.4, déjà la version en place). Portée : 13 occurrences / 11 fichiers. Remonté en tête le 2026-08-22 (balayage low-cost demandé par l'utilisateur) : mécanique, bien scopé. [Trello](https://trello.com/c/FWQDasSX)
2. **Suppression d'organisation : stats + confirmation sécurisée** — cadré le 2026-08-21 : la modale de suppression (`SuperAdminPage.tsx`) n'affiche qu'un texte générique aujourd'hui. Ajout du comptage `adherents` (absent du fetch actuel, à côté de `nb_participants`/donateurs et `nb_dons` déjà calculés) + affichage des 3 chiffres dans la modale + confirmation façon GitHub (saisie du nom exact de l'organisation pour activer le bouton Supprimer). Faible complexité, sensible (garde-fou contre suppression accidentelle de toutes les données d'une association). [Trello](https://trello.com/c/emhCr0D6)
3. **Nettoyer la dette lint** (36 erreurs/avertissements, revérifié 2026-08-20 — était 32 le 2026-08-11) — cadré : nettoyage complet en une PR, majorité (29/36) mécanique (`react-hooks/set-state-in-effect`). [Trello](https://trello.com/c/dep6US2K)
4. **Priorité 4 — Envoi par email des reçus fiscaux** — cadré le 2026-08-20 : bouton manuel individuel dans `RecusFiscauxPage.tsx`, PDF en pièce jointe via Resend, pose `recus_fiscaux.email_envoye_at`. Prérequis Resend levé depuis le 2026-08-15. [Trello](https://trello.com/c/Do9GVjOt)
