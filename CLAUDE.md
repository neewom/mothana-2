...
- Reste à cadrer/coder dans l'ordre Trello : message succès formulaire → revue UI responsive admin (bloquée sur les identifiants) → réorganisation Paramètres → vérification nom/prénom bénévole → double opt-in email → mailing Brevo. OCR scan en attente.
