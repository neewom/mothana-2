...
- Post-MVP (nouveaux champs participant) :
  ...
  - Activités : `id_externe` + `date_debut`/`date_fin` — même pattern que `profils_participant.id_externe` (traçabilité import pour rattacher les dons legacy à la bonne activité). Migration `supabase/migrations/activites_id_externe_dates.sql` (**exécutée en production le 2026-07-16**). `Activite` étendu dans `src/types/index.ts`. `ActiviteModal` : champs date début/date fin ajoutés au formulaire ; `id_externe` volontairement absent du formulaire (jamais saisi à la main, réservé à l'import — même choix que pour les participants). `ActivitesPage` : dates et référence d'import (si présente) affichées sous le nom de l'activité dans la liste.

### ⏳ Post-MVP — Reste à implémenter
- Envoi automatique des identifiants par email à l'admin créé
- Import des dons/activités legacy à partir de la liste d'activités reçue (structure `id_externe`/dates désormais en place — reste à trancher le mécanisme d'import : script ponctuel SQL/Edge Function ou saisie manuelle)
