...
- 🔄 **Export CSV reçus annuels (déclaration article 222 bis CGI)** — repris (2026-07-25) après clarification réglementaire : la déclaration ne nécessite PAS de format d'export vers un portail externe. L'obligation légale porte uniquement sur deux chiffres agrégés par organisation et par année civile (nombre de reçus émis + montant total des dons correspondants), à recopier manuellement par l'admin dans le formulaire de télédéclaration officiel (impots.gouv.fr ou demarches-simplifiees.fr selon le statut de l'organisme). Aucune donnée nominative du donateur n'est concernée. Scope retenu : nouvelle section sur `ComptabilitePage.tsx` (carte "Récapitulatif déclaratif (article 222 bis CGI)", une ligne par année avec nombre de reçus + montant total, bouton copier + export CSV optionnel via `csvExport.ts` existant), pas de nouvelle table ni migration — agrégation directe sur `recus_fiscaux` (`GROUP BY annee`, `COUNT(*)`, `SUM(montant_total)`). Checkpoint prévu après la requête d'agrégation (vérifier l'absence de double-comptage sur les reçus régénérés) avant de coder l'UI.
