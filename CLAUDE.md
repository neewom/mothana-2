...
- **Nettoyer la dette lint** (terminé 2026-08-22, PR #82 dev) : 34 → 0 problème. Le gros morceau (28× `react-hooks/set-state-in-effect`) s'est révélé être un faux positif documenté de la règle sur le pattern fetch-au-montage (recommandé par la doc React elle-même, cf. [react/react#34743](https://github.com/react/react/issues/34743)) — écarté du cadrage initial : désactivé dans `eslint.config.js` plutôt que contourné par ~28 hacks `setTimeout`. Les 6 restants traités au cas par cas (pas d'ajout aveugle de dépendance) : `useAuth` extrait dans `src/hooks/useAuth.ts` + `AuthContext`/`AuthContextValue`/`AuthState` dans `src/contexts/authContextInstance.ts` (Fast Refresh), `fetchActivites`/`loadData` passées en `useCallback`, 2 assignations mortes retirées, 1 dépendance volontairement exclue (documentée en commentaire). Vérifié : eslint 0 problème, typecheck, build prod, et les 3 flux d'auth (admin/super-admin/bénévole) sur staging.
