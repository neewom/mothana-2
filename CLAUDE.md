...
**Nouveau backlog (ajouté 2026-08-04)**
- **Incrémenter automatiquement `id_externe` à la création d'un adhérent via le formulaire** — demandé par l'utilisateur après avoir ajouté le placeholder `{{adherent_id_externe}}` sur la carte (actuellement `id_externe` n'est renseigné que pour les adhérents importés — legacy — jamais pour une création manuelle via `AdherentModal.tsx`). Explicitement mis en attente : **à cadrer/coder seulement après validation de la partie carte adhérent (PR #39)**, pas avant. Points à trancher le moment venu : format du numéro généré, unicité garantie face aux `id_externe` déjà importés (collision possible si le format se recoupe avec la numérotation de la base source), et le fait que `id_externe` n'était jusqu'ici fiable que pour les imports (règle déjà notée en mémoire persistante pour les participants) — cette évolution changerait cette donnée de fait pour toute nouvelle création côté adhérents
