...
3. **Admin : ajouter une page paramètres admin** — cadré (description complète déjà rédigée par l'utilisateur sur la carte : infos admin, reset mdp, préférences, gestion des comptes admin de l'organisation, nouveau rôle "admin root" avec droits proches du super-admin sur les admins de son organisation). [Trello](https://trello.com/c/qMqRFUoB)
4. **Cerfa : numéro de donateur fiable** (`{{numero_donateur}}`) — cadré (2026-09-15) : reprend pour `profils_participant` le pattern déjà en place pour les adhérents (`next_participant_id_externe`, appelé à la création dans `ParticipantModal.tsx`, détection collision/doublon à l'import), pas de backfill des donateurs existants. [Trello](https://trello.com/c/HpIoYMPn)
5. **Support multi-organisation pour un compte admin** (rattachement + sélecteur) — cadré, dev reporté à plus tard (décision explicite utilisateur). [Trello](https://trello.com/c/WtLrSLGW)
6. **Dette technique : factoriser CampagneCourrierPage / CampagneMailingPage** — cadré (2026-09-13), scope technique posé (hook `useDestinatairesSelection` + modale `DestinatairesApercuModal` partagée), dev volontairement reporté au 3ᵉ signal de duplication. [Trello](https://trello.com/c/RBsb8fbs)
7. **Demande d'adhésion : email aux admins à la soumission** — cadré (2026-09-15) : sur le pattern `send-demande-confirmation` existant (trigger DB + Resend), email simple "nouvelle demande à examiner" + lien, tous les admins de l'organisation par défaut (préférence d'opt-out viendra se greffer via la carte "page paramètres admin"). [Trello](https://trello.com/c/eHtWmxYK)
8. **`agents.md` calqués sur `CLAUDE.md`** (switch Codex ↔ Claude Code) — pas encore cadré, cadrage explicitement mis en pause par l'utilisateur (2026-09-13). [Trello](https://trello.com/c/0XmhK0iv)
