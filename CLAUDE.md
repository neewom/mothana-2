...
**Ancienne limitation résolue (2026-09-15)** : le super-admin en mode "Consulter" pouvait auparavant être bloqué (RPC d'import, Edge Functions reçus fiscaux/cartes adhérent/campagnes, upload d'asset) faute de `profils_organisation` dans ce contexte. Bypass étendu à toutes les Edge Functions/RPC concernées + policy RLS `organisation_assets` corrigée (PR #180). Nouveau standard pour toute policy RLS organisation-scopée : helper `is_organisation_row_accessible(organisation_id)` + fonction `audit_missing_super_admin_bypass()` à lancer après toute migration touchant RLS. Détail : `docs/journal-avancement.md`.
...
1. **Renommer "Mailing" en "Campagne mailing"** — cadré (2026-09-15), uniformisation avec "campagne-courrier" : nav, route (`/admin/adherents/mailing` → `/admin/adherents/campagne-mailing`), titre de page, FAQ, page publique "Découvrir". Complexité très faible (renommage texte/route). [Trello](https://trello.com/c/MvwdLqn9)
2. **Sortir Activités du groupe Dons dans la nav** — cadré (2026-09-15) : devient une entrée top-level du side panel (plus nichée dans "Dons"), positionnée juste au-dessus de "Comptabilité", visible si `flags.dons || flags.adherents`, nouvelle icône dédiée à créer. Complexité faible. [Trello](https://trello.com/c/DKYYxXIQ)
3. **Mire de connexion personnalisée par organisation** (admin + bénévole) — cadré. [Trello](https://trello.com/c/gkOuH3uh)
4. **Campagne mailing : inclure les donateurs** (checkbox additive "+ donateurs" sur le sélecteur existant, dédoublonnage par email avec les adhérents, opt-out symétrique sur `profils_participant`) — cadré (2026-09-13). [Trello](https://trello.com/c/GqVqTkKB)
5. **Admin : ajouter une page paramètres admin** — cadré (description complète déjà rédigée par l'utilisateur sur la carte : infos admin, reset mdp, préférences, gestion des comptes admin de l'organisation, nouveau rôle "admin root" avec droits proches du super-admin sur les admins de son organisation). [Trello](https://trello.com/c/qMqRFUoB)
6. **Cerfa : numéro de donateur fiable** (`{{numero_donateur}}`) — cadré (2026-09-15) : reprend pour `profils_participant` le pattern déjà en place pour les adhérents (`next_participant_id_externe`, appelé à la création dans `ParticipantModal.tsx`, détection collision/doublon à l'import), pas de backfill des donateurs existants. [Trello](https://trello.com/c/HpIoYMPn)
7. **Support multi-organisation pour un compte admin** (rattachement + sélecteur) — cadré, dev reporté à plus tard (décision explicite utilisateur). [Trello](https://trello.com/c/WtLrSLGW)
8. **Dette technique : factoriser CampagneCourrierPage / CampagneMailingPage** — cadré (2026-09-13), scope technique posé (hook `useDestinatairesSelection` + modale `DestinatairesApercuModal` partagée), dev volontairement reporté au 3ᵉ signal de duplication. [Trello](https://trello.com/c/RBsb8fbs)
9. **Demande d'adhésion : email aux admins à la soumission** — cadré (2026-09-15) : sur le pattern `send-demande-confirmation` existant (trigger DB + Resend), email simple "nouvelle demande à examiner" + lien, tous les admins de l'organisation par défaut (préférence d'opt-out viendra se greffer via la carte "page paramètres admin"). [Trello](https://trello.com/c/eHtWmxYK)
10. **`agents.md` calqués sur `CLAUDE.md`** (switch Codex ↔ Claude Code) — pas encore cadré, cadrage explicitement mis en pause par l'utilisateur (2026-09-13). [Trello](https://trello.com/c/0XmhK0iv)
11. **Priorité 5 — Export comptable enrichi** — recadré, roadmap lointaine. [Trello](https://trello.com/c/W3GCYUOt)
12. **Priorité 5 — Brique événements/coupons + portefeuille virtuel** (Pagode Coupon) — clarifié, prérequis (inspection projet Supabase "pagode-coupon") avant cadrage technique. [Trello](https://trello.com/c/C9A5B9jr)
13. **Priorité 5 — Gestion abonnements/plans** (facturation Samakan) — désormais rattachée à la liste Trello "Business plan — Commercialisation" (checklist commercialisation plus large créée le 2026-09-11 : pricing, juridique, marketing, support), roadmap lointaine. [Trello](https://trello.com/c/cfKF8BNw)
