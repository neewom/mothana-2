...
- ⚠️ **Assets par organisation (logo, tampon, signature) + placeholders président** — cadré et codé le 2026-08-01 sur `feat/organisation-assets-cerfa`, **en attente de test utilisateur** (pas d'identifiants admin côté agent). Décisions prises en cadrage : bucket Storage `organisation-assets` **public** (ces images finissent de toute façon sur un PDF remis au donateur, évite les URLs signées pour l'aperçu Monaco et pour Gotenberg) ; les 2 templates Cerfa par défaut (11580/16216) **non modifiés** — les 5 nouveaux placeholders (`logo_url`, `tampon_url`, `signature_url`, `president_nom`, `president_titre`) sont juste ajoutés à la liste disponible dans l'éditeur (`cerfaPreview.ts`), chaque organisation les insère elle-même si elle personnalise son template. Migration `organisation_assets_storage.sql` exécutée en prod (bucket + policy d'écriture scopée par `organisation_id`, même pattern que `recus-fiscaux`). Nouvelle sous-section "Identité visuelle" dans `ParametresPage.tsx` (Informations fiscales) : nom/titre du président (sauvegardés via le formulaire existant), 3 uploads logo/tampon/signature (PNG/JPEG, 2 Mo max, upload + sauvegarde DB immédiats dès le choix du fichier, indépendants du bouton "Enregistrer"). `generate-recu` et `ModeleRecu` (edge function + frontend) étendus avec les 5 nouveaux champs
