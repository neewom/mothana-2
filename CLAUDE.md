...
4. **Corriger le branding Mothana→Samakan dans les emails** (création admin + reset password) — cadré le 2026-08-20 : oubli du renommage du 2026-08-19, `create-admin`/`request-password-reset` envoient encore des emails "Mothana" à de vrais admins. Périmètre réduit à la correction du texte en dur, pas de nouvelle UI. Faible complexité, corrige un bug de branding en prod. [Trello](https://trello.com/c/Qf8mdFTz)
...
7. **Afficher le numéro d'adhérent dans la vérification bénévole (nom/prénom)** — cadré le 2026-08-20 : ajout de `id_externe` au retour de la RPC `search_adherents_verification` + affichage "N° adhérent : …" dans `BenevoleVerificationAdherent.tsx`, masqué si vide. Complexité faible. [Trello](https://trello.com/c/WQ4uHU2w)
8. **Identifier l'espace bénévole + l'organisation dans le header** (`/benevole`) — cadré le 2026-08-20 : header sur 2 lignes, ligne 1 inchangée ("Samakan" + Quitter), ligne 2 "Espace bénévole — *Organisation*" (même pattern de fetch que `AdminLayout.tsx`). Complexité faible. [Trello](https://trello.com/c/ZZFYupcd)
9. **Bandeau visuel distinguant l'environnement recette de la prod** — cadré le 2026-08-20 : détection par `window.location.hostname` (pas de nouvelle variable d'env), affiché dans AdminLayout ET BenevolePage, style ambre. Complexité faible. [Trello](https://trello.com/c/hSogRI1Y)
...
11. **Priorité 4 — Envoi par email des reçus fiscaux** — cadré le 2026-08-20 : bouton manuel individuel dans `RecusFiscauxPage.tsx`, PDF en pièce jointe via Resend, pose `recus_fiscaux.email_envoye_at`. Prérequis Resend levé depuis le 2026-08-15. [Trello](https://trello.com/c/Do9GVjOt)
...
15. **Nettoyer la dette lint** (36 erreurs/avertissements, revérifié 2026-08-20 — était 32 le 2026-08-11) — cadré : nettoyage complet en une PR, majorité (29/36) mécanique (`react-hooks/set-state-in-effect`). [Trello](https://trello.com/c/dep6US2K)
