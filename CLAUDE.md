...
- **Garde-fou de suppression d'organisation** (terminé 2026-08-22, PR #81 dev) : comptage `adherents` ajouté (à côté de donateurs/dons déjà calculés), modale de suppression affiche les 3 chiffres, confirmation façon GitHub (saisie du nom exact pour activer le bouton). Étendu en cours de route sur demandes de l'utilisateur : CTA "Modifier" retiré de la ligne (déjà cliquable, doublon), "Supprimer" déplacé dans le pied de la modale d'édition ; modale "Admins" fusionnée dans "Modifier l'organisation" (CTA "Admins" retiré) ; CTA "Consulter" remplacé par un picto œil ; toast de confirmation après suppression réussie (réutilise `Toast`/`useToast` existant). Vérifié de bout en bout sur staging (super-admin réel), y compris via une organisation jetable créée puis supprimée pour ne pas toucher aux données de test existantes.
