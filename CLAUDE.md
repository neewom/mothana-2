...
**Sujet "Log des modifications" + "Champ observation en cas de refus" (cartes Trello) — terminé le 2026-08-08 (PR #49 mergée)**
...
- **Cadrage de la carte "Vérifier l'ui de l'espace super admin"** (2026-08-08, sans description à l'origine, creusée avec l'utilisateur) — renommée *Revue UI responsive admin (super admin + panel org) — desktop et mobile*. Signalement : tableau des organisations qui déborde en largeur sur mobile et casse la page (encore pire modale ouverte). Cause racine confirmée par lecture de code : `SuperAdminPage.tsx` (table sans wrapper `overflow-x-auto`, contrairement à `ParticipantsPage`/`DonsPage`) — même défaut trouvé dans `AdherentsPage.tsx`. Périmètre étendu à la demande de l'utilisateur : super admin **+** tout le panel admin org (via "Consulter"). Dev non démarré, en attente des identifiants super admin (à ajouter en `.env` par l'utilisateur).
...
- Reste à cadrer/coder dans l'ordre Trello : **environnement staging** (voir ci-dessous) → revue UI responsive admin (bloquée sur les identifiants, potentiellement débloquable via staging) → réorganisation Paramètres → vérification nom/prénom bénévole → double opt-in email → mailing Brevo. OCR scan en attente.

**Environnement staging Supabase — cadré le 2026-08-09** (sujet né en conversation, sans carte Trello préexistante — [carte créée et cadrée](https://trello.com/c/MnIE0A6X)). Motivation : les Edge Functions/migrations sont actuellement testées en déployant directement en prod (aucun environnement séparé) — risque sur les données réelles des 3 organisations actives si un bug survient pendant le test, et risque de dérive silencieuse si une PR reste un temps non validée/mergée (la prod tourne alors avec du code absent de `main`). Constat technique : un seul projet Supabase actif, un `supabase/config.toml` local existe mais inutilisé, et les 37 fichiers `supabase/migrations/` ne sont pas au format horodaté CLI (appliqués à la main via `db query --linked`) — ferme la porte à une solution "Supabase Branching" native sans restructuration lourde.

**Décisions actées (design final, revu le 2026-08-09 après échange — la synchro quotidienne initialement envisagée s'est avérée inutile et risquée, remplacée par un mécanisme événementiel plus simple) :**
- Second projet Supabase permanent ("staging"), même structure que prod, palier gratuit pour démarrer (bascule vers Pro à réévaluer si limitant). Rempli de données fake — aucune donnée réelle (PII) copiée à aucun moment.
- Séparation des configs : `.env` local (`npm run dev`) pointe vers staging ; les variables d'environnement Vercel (dashboard, pas le `.env` du repo) restent inchangées vers prod — la vraie prod déployée n'est pas affectée.
- Déploiements Supabase (Edge Functions/migrations) ciblés explicitement via `--project-ref` sur chaque commande plutôt qu'un `supabase link` global qu'on pourrait oublier de re-basculer entre staging et prod.
- **Workflow par sujet nécessitant une modif base** : (1) dump de staging (snapshot avant migration) stocké localement dans un dossier gitignored du repo ; (2) migration appliquée sur staging, feature testée/validée par l'utilisateur (le local pointe déjà sur staging) ; (3) PR mergée → migration rejouée sur prod, dump supprimé, les deux bases s'alignent naturellement ; (4) PR rejetée → dump restauré sur staging (retour exact à l'état d'avant migration), migration abandonnée.
- Pas de synchro périodique : l'alignement se fait à chaque merge, suffisant tant qu'un seul sujet à la fois est testé (pratique déjà en place). Cas théorique non traité (deux migrations testées en parallèle sur staging) — non bloquant vu cette pratique.
- Comptes de test : l'utilisateur fournira des identifiants super admin dans `.env` pour staging — débloque au passage les tests de flows admin jusqu'ici impossibles côté agent (ex. `AdherentModal`).

Hors scope V1 (à trancher en codant) : contenu exact des données synthétiques (volumes de test pour reproduire d'éventuels bugs de perf), synchro des buckets Storage (pas prévue), mécanique exacte dump/restore.

Dev non démarré — confirmation explicite à redemander avant de démarrer, comme pour toute carte cadrée.
