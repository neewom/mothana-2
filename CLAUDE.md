...
1. **Page de documentation publique `/documentation`** (tour produit pour clients/prospects) — cadrée le 2026-08-19/20, sujet en cours (fil actif de la session précédente). Chemin interne `samakan.fr/documentation`, HTML soigné, captures réelles de l'app. Préalable obligatoire : purge + régénération des données de l'organisation "Association Démo Staging" (actuellement noms de vraies personnes). Prochaine étape : plan de contenu détaillé à valider avant dev. [Trello](https://trello.com/c/ngo5wJSS)
2. **Corriger le branding Mothana→Samakan dans les emails** (création admin + reset password) — cadré le 2026-08-20 : oubli du renommage du 2026-08-19, `create-admin`/`request-password-reset` envoient encore des emails "Mothana" à de vrais admins. Périmètre réduit à la correction du texte en dur, pas de nouvelle UI. Faible complexité, corrige un bug de branding en prod. [Trello](https://trello.com/c/Qf8mdFTz)
3. **Mire de connexion personnalisée par organisation** (admin + bénévole) — cadré le 2026-08-10, revérifié 2026-08-20 (toujours aligné). Même mécanique que le formulaire d'adhésion (header/footer/css éditables, URL `/connexion/{slug}` et `/login/benevole/{slug}` réutilisant le slug existant). Valeur UX/confiance multi-org. [Trello](https://trello.com/c/gkOuH3uh)
4. **Afficher le numéro d'adhérent dans la vérification bénévole (nom/prénom)** — cadré le 2026-08-20 : ajout de `id_externe` au retour de la RPC `search_adherents_verification` + affichage "N° adhérent : …" dans `BenevoleVerificationAdherent.tsx`, masqué si vide. Complexité faible. [Trello](https://trello.com/c/WQ4uHU2w)
5. **Identifier l'espace bénévole + l'organisation dans le header** (`/benevole`) — cadré le 2026-08-20 : header sur 2 lignes, ligne 1 inchangée ("Samakan" + Quitter), ligne 2 "Espace bénévole — *Organisation*" (même pattern de fetch que `AdminLayout.tsx`). Complexité faible. [Trello](https://trello.com/c/ZZFYupcd)
6. **Bandeau visuel distinguant l'environnement recette de la prod** — cadré le 2026-08-20 : détection par `window.location.hostname` (pas de nouvelle variable d'env), affiché dans AdminLayout ET BenevolePage, style ambre. Complexité faible. [Trello](https://trello.com/c/hSogRI1Y)
