...
7. **Événements/coupons + portefeuille virtuel** (Pagode Coupon) — cadré et révisé (2026-09-20) : nouveau module Mothana multi-tenant ouvert à toutes les organisations (flag désactivé par défaut), reconstruction native (pas de fusion de code, incompatibilité Next.js/Vite), **paiement en ligne dans le périmètre** (QR livré seulement après validation du paiement ; prestataire/modèle d'encaissement décidé par la carte 3). Découpé en 8 cartes dans la liste Trello "Coupon" : fondations → admin événement → cadrage paiement → paiement backend → achat public + QR → durcissement `verify-pin` → scan bénévole → dashboard admin. Confirmation à redemander avant de démarrer la première. [Trello](https://trello.com/c/C9A5B9jr)
